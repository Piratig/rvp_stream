...
def df_update(line_name, df):
    ...
    df = df.drop_duplicates(['route'], keep='first')
...
def db_query():
    cursor = conn.cursor()
    df_rvp_query = pd.read_sql("SELECT line, route, headnum, tcs, datearrival FROM train ORDER BY id DESC LIMIT 1000",
                               conn)
...
app.layout = html.Div(children=[
    ...
    dt.DataTable(id='tbl', style_data={'whiteSpace': 'normal', 'height': 'auto', },
                 ...
                 style_cell_conditional=[{'if': {'column_id': 'line'}, 'width': '5%'},
                                         {'if': {'column_id': 'route'}, 'width': '5%'},
                                         {'if': {'column_id': 'headnum'}, 'width': '5%'}]
                 ),
